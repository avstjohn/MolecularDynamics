...
int main(int argc, char *argv[])
{
  ...
  cout << endl;
    ...
    outStream3 << "tsb35-c" << nCarbon << "_0" << nQuarter << "-quarter_" << guestSegname << "-PDB.pdb";
  ...
  OUT3 << "CRYST1  " << Bmag << "  " << Bmag << "  100.000  " << domainAngle << "  " << domainAngle << "  " << domainAngle << "          P 1           1" << endl;
  for (int i=0; i<nMolecules1; i++)
  {
    for (int j=0; j<nMolecules2; j++)
    {
...
//       OUT3 << "ATOM " << setw(6) << k + moleculeCount*nAtomsPerGuest + 1;
       OUT3 << "ATOM " << setw(6) << k - nAtomsPerTSB + moleculeCount*nAtomsPerGuest + 1;
       OUT3 << " " << setw(4) << Atom[k]->getAtomName() << " COR   " << setw(3) << moleculeCount+1 << "     ";
       OUT3 << setw(7) << position.x << " " << setw(7) << position.y << " " << setw(7) << position.z << "  ";
